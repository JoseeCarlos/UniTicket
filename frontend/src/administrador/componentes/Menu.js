...
const Submenu = (props) => {
    ...
    const renderizarEnlace = (elemento, i) => {
        ...
        if (elemento.to) {
            return (
                <NavLink aria-label={elemento.label} onKeyDown={enTeclaAbajo} role="menuitem" className="p-ripple" activeClassName="router-link-active router-link-exact-active" to={elemento.to} onClick={(e) => enMenuElementoClic(e, elemento, i)} exact target={elemento.target}>
                    {contenido}
                </NavLink>
            )
        }
            ...
            return (
                <a tabIndex="0" aria-label={elemento.label} onKeyDown={enTeclaAbajo} role="menuitem" href={elemento.url} className="p-ripple" onClick={(e) => enMenuElementoClic(e, elemento, i)} target={elemento.target}>
                    {contenido}
                </a>
            );
        }
            ...
            return (
                <li className={styleClass} key={i} role="none">
                    {props.root === true && <React.Fragment>
                        ...
                        <Submenu elementos={elemento.items} enMenuElementoClic={props.enMenuElementoClic} />
                    </React.Fragment>}
                </li>
            );
        }
