...
class AsignacionModelo():
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                for row in cursor.fetchall():
                    asignaciones.append(AsignacionEmpleado(IdEmpleado=row[0], NombreArea=row[1], NombreLugar=row[2], IdMesa=row[3], NumeroMesa=row[4], FechaRegistro=row[5], FechaModificacion=row[6]).to_JSON())
            connection.close()
            return asignaciones
    ...
    #         connection = get_connection()
    #         with connection.cursor() as cursor:
    #             cursor.execute("""INSERT INTO asignacion (employeeId, tableId, number, createDate, updateDate)
    #                                 VALUES (%s, %s, %s, %s, %s)
    #                             """, (asignacion.employeeId, asignacion.tableId, asignacion.number, asignacion.createDate, asignacion.updateDate))
    #             connection.commit()
    #         connection.close()
    #         return jsonify({"message": "Asignacion creada con exito"}), 200
    #     except Exception as ex:
    #         raise Exception(ex)
