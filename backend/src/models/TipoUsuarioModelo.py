...
class TipoUsuarioModelo():
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                for row in cursor.fetchall():
                    tipoUsuario.append(TipoUsuario(idTipoUsuario=row[0],Nombre=row[1],Descripcion=row[2], IdUsuarioRegistro=row[3], Estado=row[4], FechaRegistro=row[5], FechaModificacion=row[6]).to_JSON())
            connection.close()
            return tipoUsuario
    ...
    def crear_TipoUsuario(self, tipoUsuario):
        try:
            connection = get_connection()
            with connection.cursor() as cursor:
                cursor.execute("""INSERT INTO UTipoUsuario(Nombre, Descripcion,  IdUsuarioRegistro)
                                    VALUES (?, ?, ?)
                                """, (tipoUsuario.Nombre, tipoUsuario.Descripcion, tipoUsuario.IdUsuarioRegistro))
                connection.commit()
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("""UPDATE UTipoUsuario
                                    ...
                                    WHERE IdTipoUsuario=?
                                """, (tipoUsuario.Nombre, tipoUsuario.Descripcion, tipoUsuario.FechaModificacion, tipoUsuario.idTipoUsuario))
                connection.commit()
                affected_rows = cursor.rowcount
            connection.close()
            return affected_rows
    ...
    @classmethod
    def eliminar_TipoUsuario(self, tipoUsuarioId):
        try:
            connection = get_connection()
            with connection.cursor() as cursor:
                cursor.execute("""UPDATE UTipoUsuario
